...
# store_load's ignored test reads each compiled-in checkpoint's real model.safetensors
# header from the HF cache, downloading what is missing
cargo test --no-default-features \
  --features "backend-flex,backend-simd,mamba1,mamba2,mamba3-siso,mamba3-mimo" \
...
│  │                      it too); tests replay every compiled-in checkpoint's vendored
│  │                      safetensors manifest (globals + layer 0) through `key_remapping`
│  │                      and require set equality with the built module's params, no two
│  │                      names colliding and no rule matching nothing; an `#[ignore]`d
│  │                      test re-reads the real headers to prove the manifests complete
...
- **Key remapping** rewrites `backbone.layers.{i}.mixer.X` →
  ...
  `mimo_{x,z,o}` → `mimo_{x,z,o}_hmp` (only at `mimo_rank > 1`, where the module has
  them); it has no `A_log` (A is data-dependent), and its
